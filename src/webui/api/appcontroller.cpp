...
void AppController::preferencesAction()
{
    ...
    // Saving Management
    data["auto_tmm_enabled"] = session->isAutoTMMDisabledByDefault() ? 0 : 1;
...
void AppController::setPreferencesAction()
{
    ...
    // Downloads
    // When adding a torrent
    if (m.contains("create_subfolder_enabled"))
        session->setCreateTorrentSubfolder(m["create_subfolder_enabled"].toBool());
    if (m.contains("start_paused_enabled"))
        session->setAddTorrentPaused(m["start_paused_enabled"].toBool());
    if (m.contains("auto_delete_mode"))
        TorrentFileGuard::setAutoDeleteMode(static_cast<TorrentFileGuard::AutoDeleteMode>(m["auto_delete_mode"].toInt()));
    ...
    // Saving Management
    if (m.contains("auto_tmm_enabled"))
        session->setAutoTMMDisabledByDefault(!m["auto_tmm_enabled"].toBool());
    if (m.contains("torrent_changed_tmm_enabled"))
        session->setDisableAutoTMMWhenCategoryChanged(!m["torrent_changed_tmm_enabled"].toBool());
    if (m.contains("save_path_changed_tmm_enabled"))
    ...
    if (m.contains("mail_notification_sender"))
        pref->setMailNotificationSender(m["mail_notification_sender"].toString());
    ...
    if (m.contains("slow_torrent_dl_rate_threshold"))
        session->setDownloadRateForSlowTorrents(m["slow_torrent_dl_rate_threshold"].toInt());
    if (m.contains("slow_torrent_ul_rate_threshold"))
        session->setUploadRateForSlowTorrents(m["slow_torrent_ul_rate_threshold"].toInt());
    if (m.contains("slow_torrent_inactive_timer"))
        session->setSlowTorrentsInactivityTimer(m["slow_torrent_inactive_timer"].toInt());
    // Share Ratio Limiting
