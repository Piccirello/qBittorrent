...
QStringList SearchEngineWeb::supportedCategories() const
{
    QStringList result;
    foreach (const PluginInfo *plugin, m_plugins.values()) {
        if (plugin->enabled) {
...
PluginInfo *SearchEngineWeb::pluginInfo(const QString &name) const
{
...
void SearchEngineWeb::enablePlugin(const QString &name, bool enabled)
{
...
void SearchEngineWeb::updatePlugin(const QString &name)
{
    installPlugin(QString("%1%2.py").arg(m_updateUrl).arg(name));
}

// Install or update plugin from file or url
void SearchEngineWeb::installPlugin(const QString &source)
{
    ...
    if (Utils::Misc::isUrl(source)) {
        ...
        connect(handler, static_cast<void (DownloadHandler::*)(const QString &, const QString &)>(&DownloadHandler::downloadFinished)
                , this, &SearchEngineWeb::pluginDownloaded);
        connect(handler, &DownloadHandler::downloadFailed, this, &SearchEngineWeb::pluginDownloadFailed);
    }
    else {
        QString path = source;
...
void SearchEngineWeb::installPlugin_impl(const QString &name, const QString &path)
{
...
bool SearchEngineWeb::uninstallPlugin(const QString &name)
{
...
void SearchEngineWeb::updateIconPath(PluginInfo * const plugin)
{
    if (!plugin) return;
...
void SearchEngineWeb::checkForUpdates()
{
    ...
    connect(handler, static_cast<void (DownloadHandler::*)(const QString &, const QByteArray &)>(&DownloadHandler::downloadFinished)
            , this, &SearchEngineWeb::versionInfoDownloaded);
    connect(handler, &DownloadHandler::downloadFailed, this, &SearchEngineWeb::versionInfoDownloadFailed);
}
...
QString SearchEngineWeb::pluginFullName(const QString &pluginName)
{
...
void SearchEngineWeb::versionInfoDownloaded(const QString &url, const QByteArray &data)
{
    Q_UNUSED(url)
    parseVersionInfo(data);
}

void SearchEngineWeb::versionInfoDownloadFailed(const QString &url, const QString &reason)
{
    Q_UNUSED(url)
    emit checkForUpdatesFailed(tr("Update server is temporarily unavailable. %1").arg(reason));
}

void SearchEngineWeb::pluginDownloaded(const QString &url, QString filePath)
{
...
void SearchEngineWeb::pluginDownloadFailed(const QString &url, const QString &reason)
{
...
void SearchEngineWeb::updateNova()
{
...
void SearchEngineWeb::readSearchOutput()
{
    ...
    foreach (const QByteArray &line, output.split('\n')) {
        // qDebug() << "Buffer size:" << m_stdoutBuffer.size();
        // m_stdoutBuffer.write(line.data());

        SearchResult searchResult;

        if (parseSearchResult(QString::fromUtf8(line), searchResult))
            // m_stdoutBuffer.write(searchResult.fileUrl);
...
void SearchEngineWeb::parseVersionInfo(const QByteArray &info)
{
...
bool SearchEngineWeb::isUpdateNeeded(QString pluginName, PluginVersion newVersion) const
{
